...
void random_aead_gen(unsigned char *c,	
    ...
    *odd_ad_blocks = (unsigned long long) rand()%2;   
    ...
    for(i=0; i<28*(*adlen); i++) ad[i] = rand() % 256;
    if ((*odd_ad_blocks == 1) && (*adlen >0)) for(i = 28*(*adlen)-12; i<28*(*adlen); i++) ad[i] = 0x00;
    
	unsigned long long m_len = (*mlen) * 16;
	unsigned long long ad_len = 0;
	if (*adlen > 0) 
	    ad_len = (*adlen) * 28 - 12*(*odd_ad_blocks);
    unsigned long long clen;
    // make ad half-complete
    if (*incomplete_ad == 0x01 && *adlen >0) {
        if (*odd_ad_blocks == 0) {
        	for(i=28*(*adlen) - 4; i<28*(*adlen); i++) ad[i] = 0x00;
        	ad[28*(*adlen)-1]= 0x08; // this is romulus padding for 8 bytes
		    ad_len -= 4;
		} else {
		    for(i=28*(*adlen) - 8 - 12; i<28*(*adlen) - 12; i++) ad[i] = 0x00;
        	ad[28*(*adlen)-1 - 12]= 0x08; // this is romulus padding for 8 bytes
		    ad_len -= 8;
		}
	}
...
void save_aead(FILE *in, FILE* out,
	...
	fprintf(in, "%d\n", (int) *odd_ad_blocks);
	fprintf(in, "%d\n", (int) *incomplete_m);
	for(i=0;i<12;i++) fprintf(in, "%02X", npub[i]); fprintf(in, "\n");
	for(i=0;i<16;i++) fprintf(in, "%02X", k[i]); fprintf(in, "\n");
	for(j=0;j<*adlen;j++) {
	    for(i=28*j;i<28*(j+1);i++) 
	        fprintf(in, "%02X", ad[i]); 
        fprintf(in, "\n"); 
    }
