...
#ifdef DEBUG
    fic = fopen("SKINNY_TBC_detailed_TV.txt", "w");
#endif
...
int main() {
    ...
    int i;
    uint8_t tweakey[48] = {0x4f, 0x55, 0xcf, 0xb0, 0x52, 0x0c, 0xac, 0x52, 0xfd, 0x92, 0xc1, 0x5f, 0x37, 0x07, 0x3e, 0x93, 0x4f, 0x55, 0xcf, 0xb0, 0x52, 0x0c, 0xac, 0x52, 0xfd, 0x92, 0xc1, 0x5f, 0x37, 0x07, 0x3e, 0x93, 0x4f, 0x55, 0xcf, 0xb0, 0x52, 0x0c, 0xac, 0x52, 0xfd, 0x92, 0xc1, 0x5f, 0x37, 0x07, 0x3e, 0x93};
    uint8_t input[16] = {0xf2, 0x0a, 0xdb, 0x0e, 0xb0, 0x8b, 0x64, 0x8a, 0x3b, 0x2e, 0xee, 0xd1, 0xf0, 0xad, 0xda, 0x14};//{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};//
    uint8_t output[16];
    const int ver = 5;
    ...
    for(int j=0; j<100; j++) {
       for(i=0; i<KEYSIZE; i++) tweakey[i] = rand() % 256;
       for(i=0; i<KEYSIZE ;i++) {
        fprintf(testinput, "%02X", tweakey[i]); 
        if(i % 16 == 15)
            fprintf(testinput, "\n");
       }
       for(i=0; i<BLOCKSIZE; i++) input[i] = rand() % 256;
       for(i=0; i<BLOCKSIZE ;i++) fprintf(testinput, "%02X", input[i]); fprintf(testinput, "\n");
       enc(input, tweakey, output, ver);
       for(i=0; i<BLOCKSIZE ;i++) fprintf(testoutput, "%02X", output[i]); fprintf(testoutput, "\n");
    }
